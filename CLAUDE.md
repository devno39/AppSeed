...
- **Storage has 3 hard rules** (policies OR together so one bucket-wide policy cancels the narrow ones, the owner id goes in the object path from day one, a file's path is queued *before* the row naming it is deleted) — read `supabase/README.md` before touching buckets or policies.
- **Remote config the splash reads must be public.** The version gate runs before auth, so an `authenticated`-only read policy silently skips it for signed-out users — the ones a forced update is for. `app_config.is_public` exists for that row and no other.
- **No prompt presents on its own at launch.** Permissions, What's New, the onboarding paywall, the review ask and the scheduled paywall all go through `LaunchPromptManager` — the order and the "is it due" rules live there, scenes only register *how* to present. Adding a launch-time sheet means adding a `LaunchPrompt` case, not a `viewDidAppear` check. Three rules the registrations encode: count on presentation (not before it), report the dismissal so the next one can run, and settle a prompt via `!shouldShow()` rather than a "has been shown" flag. Read `AppSeed/Helpers/README.md` → Launch prompts. Trim `PermissionType` to what the app actually asks for: `allGranted` requires every case.
